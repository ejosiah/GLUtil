...
float sampleCloudDensity(vec3 p, Weather weather){
	...
	base_cloud = remap(base_cloud, weather.cloud_coverage, 1.0, 0., 1.0); // fake cloud coverage
...
//	float perlinWorley = texture(noise, vec3(uv * 0.5, z)).x;
//	vec3 worley = texture(noise, pos).yzw;
//
